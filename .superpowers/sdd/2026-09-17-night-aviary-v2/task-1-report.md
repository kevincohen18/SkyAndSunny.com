...
- Responsive-gap correction: `378e299407390b57316a3bbb2c1948e0385cfb34` (`fix: bridge aviary responsive composition gap`)
...
- Sunny's visible toes rest on the shared branch. Breakpoint-specific tapered junctions reduce Sky's measured extended-foot/perch distance to 0.6px at 1440, 0.6px at 800, 2.2px at 900, and 3.3px at 390 without entering either face/title clearing.
...
- `verification/night-aviary-first-viewport-900.png`
- `verification/night-aviary-js-off-mobile.png`
- `verification/night-aviary-reduced-motion-mobile.png`

The final responsive-gap correction modified only `sandbox/night-aviary/index.html`, `sandbox/night-aviary/styles.css`, `tests/verify-night-aviary.sh`, this report, and the new exact 900×900 evidence image. It did not alter root `index.html`, JavaScript, media assets, mappings, or the approved endpoint compositions.
...
For the final responsive-gap correction, the expanded sweep first reproduced the review: two Sunny-over-title samples failed at every width from 833 through 1023; Sky contact measured 47.5/43.6/32.2/20.5/8.8/5.1px at 833/850/900/950/1000/1023; and the Obsessions buffer had 9/9/5/3/0/0 sampled branch entries. The bounded 833–1023 rule and tapered junction turned those same assertions green without changing the endpoint layouts.

- `bash tests/verify-night-aviary.sh` — pass: `Night Aviary V2 structural checks passed`.
- The verifier now requires the two transparent cutout derivatives, direct cutout rendering, alpha when `webpinfo` is available, a 400KB per-cutout budget, exact TikTok mappings, the habitat structure, three distinct compound leaf symbols, desktop/intermediate/responsive-gap Sky junctions, the compact connecting branch, motion lifecycle hooks, face/caption hooks, and absence of the rejected V1/mask/font claims.
...
- Exact first viewports: 1440×900, 900×900, 800×900, and 390×844 — no horizontal overflow, broken image, failed request, or console error.
...
- Five-point face hit-test grids pass for both residents at 1440×900, 800×900, every intermediate sweep width, 390×844, JS-off 390×844, reduced-motion 390×844, and 320×800. True face centers were sampled (not generic image centers). Representative centers: desktop Sunny `(855,248)`, Sky `(1194,193)`; 800px Sunny `(212,303)`, Sky `(626,309)`; mobile Sunny `(157,296)`, Sky `(270,276)`.
- Continuous-branch sampling at 3-unit SVG intervals with a 14px caption buffer reports zero collisions at every rendered state. On-canvas sample share is 97.4% at 1440, 96.4–96.6% throughout the 833–1023 sweep, and 24.1% at both 800 and 390, clearing the explicit 20% counter-assertion and replacing the reviewed 0% compact-width result.
- Sky/perch geometry uses the supplied cutout's true extended-foot coordinate and samples every visible shared-perch path at 2-unit intervals. Nearest distances are 0.6px at 1440, 0.6px at 800, 2.2/2.2/2.2/2.3/2.3/0.9px at 833/850/900/950/1000/1023, and 3.3px at 390 against a 10px maximum.
- The deterministic intermediate-width sweep at 833, 850, 900, 950, 1000, and 1023px reports zero title topmost-hit failures, zero resident-over-title hits, clear five-point face probes, zero 14px caption-buffer collisions, no horizontal overflow, no console errors, no failed requests, and meaningful branch presence at every width.
...
- All seven sandbox WebP assets total approximately 1.1MB on disk. HTML/CSS/JS total 45,610 bytes uncompressed.
...
- `verification/night-aviary-first-viewport-900.png` — exact 900×900 responsive-gap evidence.
...
- Safe zones: pass for the recorded states, including every sampled width from 833 through 1023px. The birds and vegetation do not cover title/nav/copy/actions; title topmost hits, true face hit targets, and caption buffers are programmatically clear.
